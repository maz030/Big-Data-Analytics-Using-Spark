...
# Your code here
with open('../../Data/hw6-files-final.txt') as path:
    files = [line.strip() for line in path.readlines()]

text_file=sc.textFile(','.join(files)).cache()
...
n=pairlist.map(lambda x : (x[0],1)).reduceByKey(lambda x,y:x+y).cache()   
print_count(n)
