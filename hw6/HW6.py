...
# Your code here
path = open('../../Data/hw6-files-10mb.txt').read().strip()
...
valid_objects = text_file.map(lambda x: safe_parse(x)).filter(lambda x: x != None)                  .map(lambda x: (x['user']['id_str'], x['text'].encode('utf-8'))).cache()
...
# your code here
tokenlist = valid_objects.flatMap(lambda x: [(token, x[0]) for token in set(tok.tokenize(x[1]))]).distinct().cache()
num = tokenlist.map(lambda x: (x[0], 1)).reduceByKey(lambda x,y: x+y).cache()
...
# your code here
token_filter = num.filter(lambda x: x[1] >= 100).cache()
result = token_filter.map(lambda (x,y): (y,x)).sortByKey().map(lambda (x,y): (y,x)).cache()
print_count(result)
...
# your code here
valid_tokenlist = list(token_filter.map(lambda x: x[0]).collect())
group_tokenlist = tokenlist.filter(lambda x: x[0] in valid_tokenlist).cache();
token_group = group_tokenlist.map(lambda x: (List.get(x[1]), x[0])).map(convertKey)             .map(lambda x: (x, 1)).reduceByKey(lambda x, y: x+y)             .map(lambda x: (x[0][1], (x[0][0], x[1]))).cache()
...
for i in range(8):
    output = rel.filter(lambda x: x[0] == i).map(lambda x: (x[2], x[1])).sortBy(lambda x: (-x[1], x[0]))
